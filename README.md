...
| Incoming Call Notification | Users receive a real-time incoming call notification overlay across the application interface. The notification displays the caller name and provides accept and decline controls for quick interaction. | ![alt text](assets/screenshots/CALL-NOTIFICATION-ON_NEARBY.png) |
| Chat Call Notification | Incoming calls also appear directly inside the chat interface, allowing users to answer or reject calls without leaving the conversation page. To manage server resources, security considerations, and operating costs during the initial release, voice calls are currently limited to 1 minute per call and 5 call attempts per user per day. These restrictions were implemented as a precautionary measure and may be adjusted or removed in future versions as the platform evolves. Because these limits are temporary implementation decisions rather than core user-facing features, dedicated notifications informing users about the restrictions have not been implemented at this stage. | ![alt text](assets/screenshots/CALL-NOTIFICATION-ON_CHAT-PAGE.png) |
| Call Modal | The private chat interface includes a dedicated call modal with caller information and accept or decline actions to simulate a modern mobile calling experience. | ![alt text](assets/screenshots/CHAT_CALL_MODAL.png) |
...
| User Confirmation Notifications | Custom notification banners provide immediate feedback for important account-related actions, including login, logout, and other successful user interactions, enhancing the overall user experience. | ![User Confirmation Notifications](assets/screenshots/MESSAGE_INFO.png) |
